...
def calculate_national_metrics(conn, quarter):
    ...
    SELECT
        ...
        SUM(MDScensus) as Total_Resident_Days, -- Renamed from Total_Residents
    ...
    try:
        ...
        return {
            ...
            'Total_Resident_Days': float(result[1]), # Renamed key
...
def calculate_median_contract_percentage(conn, quarter):
    """Calculate median contract staff percentage for a quarter"""
    query = f"""
    ...
    try:
        ...
        print(f"Error calculating median contract percentage for quarter {quarter}: {str(e)}")
        return 0.0

def main():
    print("Starting national metrics generation...")
    ...
    try:
        ...
        cursor = conn.cursor()
        # Extract quarter from the WorkDate field
        cursor.execute("""
        ...
        for quarter in quarters:
            ...
            if metrics:
                ...
                print(f"Successfully calculated metrics for {quarter}")
            else:
                print(f"No metrics calculated for {quarter}")

        if not national_metrics:
            print("No metrics were calculated. Please check the data and calculations.")
            return

        df = pd.DataFrame(national_metrics)
        df.to_csv('national_quarterly_metrics.csv', index=False)
        print("\nnational_quarterly_metrics.csv generated successfully.")
        print(f"Generated metrics for {len(quarters)} quarters.")
